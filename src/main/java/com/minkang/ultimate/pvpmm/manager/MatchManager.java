...
import com.minkang.ultimate.pvpmm.Main; import com.minkang.ultimate.pvpmm.model.Arena; import com.minkang.ultimate.pvpmm.model.Match; import com.minkang.ultimate.pvpmm.util.Elo;
import org.bukkit.*; import org.bukkit.entity.Player; import org.bukkit.plugin.Plugin; import org.bukkit.scheduler.BukkitRunnable; import org.bukkit.scoreboard.*;
import java.util.*;
public class MatchManager {
    private final Plugin plugin; private final com.minkang.ultimate.pvpmm.service.TeamService teamService;
    private final Map<java.util.UUID, Integer> rating = new HashMap<java.util.UUID, Integer>(); private final Map<java.util.UUID, Integer> wins = new HashMap<java.util.UUID, Integer>(); private final Map<java.util.UUID, Integer> losses = new HashMap<java.util.UUID, Integer>();
    private final Map<String, Match> active = new HashMap<String, Match>(); private final Map<java.util.UUID, String> playerToMatch = new HashMap<java.util.UUID, String>();
    ...
    public void startMatch(Arena arena, Set<java.util.UUID> teamA, Set<java.util.UUID> teamB){
        String id="M"+System.currentTimeMillis(); Match m=new Match(id, arena, teamA, teamB); active.put(id,m);
        for(java.util.UUID u: teamA) playerToMatch.put(u, id); for(java.util.UUID u: teamB) playerToMatch.put(u, id);
        for(java.util.UUID u: teamA) snapshotPlayer(u, m); for(java.util.UUID u: teamB) snapshotPlayer(u, m);
        teleportTeam(teamA, arena.getPos(1), arena.getPos(2)); teleportTeam(teamB, arena.getPos(3), arena.getPos(4));
    ...
    private void setupScoreboardTeams(Match m){
        ...
        String hash=Integer.toHexString(m.getId().hashCode()); String ta=("pA"+hash), tb=("pB"+hash); if(ta.length()>16) ta=ta.substring(0,16); if(tb.length()>16) tb=tb.substring(0,16);
    ...
    public void endMatch(Match m, boolean teamAWon){
        if(m.getState()==Match.State.ENDED) return; m.setState(Match.State.ENDED);
        ...
        boolean underdogA=(rA+50.0<rB), underdogB=(rB+50.0<rA);
        ...
        double sA=teamAWon?1.0:0.0, sB=teamAWon?0.0:1.0; int dA=(int)Math.round(kA*(sA-expA)); int dB=(int)Math.round(kB*(sB-expB));
        Bukkit.broadcastMessage("§d[경쟁전] §f매치 종료! §7승자: "+(teamAWon?"§a"+formatTeam(m.getTeamA()):"§c"+formatTeam(m.getTeamB())));
        for(java.util.UUID u:m.getTeamA()){ int before=getRating(u); int after=before+dA; int min=Main.get().getConfig().getInt("rating.min",0), max=Main.get().getConfig().getInt("rating.max",3000); if(after<min) after=min; if(after>max) after=max; setRating(u, after); if(teamAWon) wins.put(u, getWins(u)+1); else losses.put(u, getLosses(u)+1); Player p=Bukkit.getPlayer(u); if(p!=null) p.sendMessage("§d[경쟁전] §a"+(teamAWon?"승리":"패배")+"§7 | 레이팅 §f"+before+" §7→ §e"+after+" §7("+(after-before)+")"); }
        for(java.util.UUID u:m.getTeamB()){ int before=getRating(u); int after=before+dB; int min=Main.get().getConfig().getInt("rating.min",0), max=Main.get().getConfig().getInt("rating.max",3000); if(after<min) after=min; if(after>max) after=max; setRating(u, after); if(!teamAWon) wins.put(u, getWins(u)+1); else losses.put(u, getLosses(u)+1); Player p=Bukkit.getPlayer(u); if(p!=null) p.sendMessage("§d[경쟁전] §a"+(!teamAWon?"승리":"패배")+"§7 | 레이팅 §f"+before+" §7→ §e"+after+" §7("+(after-before)+")"); }
        ...
        cleanupScoreboard(m); forceUnmapAll(m); if(Main.get().getConfig().getBoolean("match.clearTeamOnEnd", true)) clearTeams(m); active.remove(m.getId());
    }
    ...
    public void playerDiedOrQuit(java.util.UUID id){
        Match m=getMatch(id); if(m==null) return; boolean wasA=m.getTeamA().remove(id); if(!wasA) m.getTeamB().remove(id); playerToMatch.remove(id);
    ...
    private void performWeeklyReset(){ Bukkit.broadcastMessage("§6[경쟁전] §f주간 시즌 종료! 등급 보상을 지급합니다."); for(java.util.UUID u: new java.util.HashSet<java.util.UUID>(rating.keySet())){ rating.put(u, Main.get().getConfig().getInt("rating.start",1000)); wins.put(u,0); losses.put(u,0);} }
