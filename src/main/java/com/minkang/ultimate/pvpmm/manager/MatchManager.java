...
public class MatchManager {
    ...
    public void endMatch(Match m, boolean teamAWon){
        ...
        StringBuilder sA=new StringBuilder(); for(java.util.UUID u:m.getTeamA()){ org.bukkit.OfflinePlayer op=Bukkit.getOfflinePlayer(u); sA.append(" ").append(op!=null?op.getName():"Unknown").append("§7(").append(getRating(u)).append(")"); }
        StringBuilder sB=new StringBuilder(); for(java.util.UUID u:m.getTeamB()){ org.bukkit.OfflinePlayer op=Bukkit.getOfflinePlayer(u); sB.append(" ").append(op!=null?op.getName():"Unknown").append("§7(").append(getRating(u)).append(")"); }
        Bukkit.broadcastMessage("§d[경쟁전] §f레이팅 변동§7 - §aA:"+sA.toString()+" §7| §cB:"+sB.toString());
