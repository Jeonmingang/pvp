...
public class ArenaManager {
    ...
    public boolean deleteByIndex(int oneBasedIndex) {
        ...
        java.util.Collections.sort(list, new java.util.Comparator<Arena>() {
            public int compare(Arena a, Arena b) { return a.getName().compareToIgnoreCase(b.getName()); }
        });
        int idx = oneBasedIndex - 1;
        if (idx < 0 || idx >= list.size()) return false;
        Arena target = list.get(idx);
        arenas.remove(target.getName().toLowerCase(java.util.Locale.ROOT));
        save();
        return true;
    }
}
