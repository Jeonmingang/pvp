...
public class CompetitionCommand implements CommandExecutor {
    ...
    public CompetitionCommand(Main plugin, ArenaManager arenaManager, Matchmaker matchmaker, TeamService teamService, InviteService inviteService, RewardManager rewardManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if ("설정".equals(args[0])) {
            if (!sender.hasPermission("pvpmm.admin")) {
        ...
        if ("목록".equals(args[0])) {
            ...
            if (list.isEmpty()) {
                sender.sendMessage("§7[경쟁전] 등록된 경기장이 없습니다.");
            } else {
                sender.sendMessage("§a[경쟁전] 경기장 목록:");
                for (Arena a : list) {
                    sender.sendMessage(" - " + a.getName() + (a.isReady() ? " §7(준비완료)" : " §c(미완성)"));
                }
            }
            return true;
        }
    ...
    private boolean handleAdmin(CommandSender sender, String[] args) {
        ...
        if ("경기장".equals(args[1])) {
            ...
            if (args.length >= 3 && "위치".equals(args[2])) {
                if (!(sender instanceof Player)) {
                    sender.sendMessage("플레이어만 가능합니다.");
                    return true;
                }
                if (args.length < 5) {
                    sender.sendMessage("§e/경쟁전 설정 경기장 위치 <이름> <1~4>");
                ...
                String name = args[3];
                int idx;
                try { idx = Integer.parseInt(args[4]); } catch (Exception e) { sender.sendMessage("§c숫자를 입력하세요 (1~4)"); return true; }
                if (idx < 1 || idx > 4) { sender.sendMessage("§c1~4 범위만 허용됩니다."); return true; }
                Location loc = p.getLocation().clone();
                boolean ok = arenaManager.setPos(name, idx, loc);
                if (!ok) {
                    sender.sendMessage("§c[경쟁전] 해당 이름의 경기장이 없습니다.");
                } else {
                    sender.sendMessage("§a[경쟁전] " + name + " 위치 " + idx + " 저장 완료.");
                }
                return true;
            }
        }
    ...
    private boolean handleTeam(Player p, String[] args) {
        ...
        if ("수락".equals(args[1])) {
            inviteService.accept(p);
            return true;
        }
        if ("거절".equals(args[1])) {
            inviteService.deny(p);
            return true;
        }
    ...
    private void sendHelp(CommandSender s) {
        ...
        s.sendMessage("§e/경쟁전 팀 수락 | /경쟁전 팀 거절");
