...
public class Matchmaker {
    ...
    public boolean enqueue(Player p) {
        if (matchManager.isInMatch(p.getUniqueId())) {
            p.sendMessage("§c[경쟁전] 이미 매치 중입니다.");
            return false;
        }
        if (enqueuedAt.containsKey(p.getUniqueId())) {
            p.sendMessage("§c[경쟁전] 이미 매칭 대기 중입니다. /경쟁전 매치 취소");
            return false;
        }
        if (teamService.hasTeam(p)) {
            Set<java.util.UUID> team = teamService.getTeam(p.getUniqueId());
            if (team == null) {
                team = teamService.getOrCreateSolo(p);
            }
            if (team.size() > 2) {
                p.sendMessage("§c[경쟁전] 팀 인원 초과.");
                return false;
            }
            // ensure both are not already queued
            boolean ok = true;
            for (java.util.UUID m : team) {
                if (enqueuedAt.containsKey(m)) ok = false;
            }
            if (!ok) {
                p.sendMessage("§c[경쟁전] 팀원 중 대기 중인 인원이 있습니다.");
                return false;
            }
    ...
    public boolean cancel(Player p) {
        ...
        if (!removed) {
            // maybe part of a duo queue
            Set<java.util.UUID> found = null;
            for (Set<java.util.UUID> s : duoQueue) {
                if (s.contains(p.getUniqueId())) {
                    found = s;
                    break;
                }
            }
        ...
        if (removed) {
            Bukkit.broadcastMessage("§7[경쟁전] " + p.getName() + " §7매칭 대기 취소");
        }
        return removed;
    }

    private void tick() {
        // first, make duo vs duo
        ...
        while (soloQueue.size() >= 2) {
            ...
            Set<java.util.UUID> sa = new HashSet<java.util.UUID>();
            sa.add(a);
            Set<java.util.UUID> sb = new HashSet<java.util.UUID>();
            sb.add(b);
            startAnyMatch(sa, sb);
        }

        // 1v2 allowed if wait exceeded
        if (!soloQueue.isEmpty() && !duoQueue.isEmpty()) {
            java.util.UUID solo = soloQueue.peek();
            Long since = enqueuedAt.get(solo);
            if (since == null) since = Long.valueOf(System.currentTimeMillis());
            long waited = System.currentTimeMillis() - since.longValue();
            int need = Main.get().getConfig().getInt("rating.mismatchWaitSeconds", 45) * 1000;
            if (waited >= need) {
                soloQueue.poll();
                Set<java.util.UUID> sa = new HashSet<java.util.UUID>();
                sa.add(solo);
                Set<java.util.UUID> duo = duoQueue.poll();
                startAnyMatch(sa, duo);
            }
        }
    }
    ...
    private void startAnyMatch(Set<java.util.UUID> a, Set<java.util.UUID> b) {
        ...
        if (arena == null || !arena.isReady()) {
            // can't start; push back into queues
            if (a.size() == 1) soloQueue.addAll(a);
            else duoQueue.add(a);
            if (b.size() == 1) soloQueue.addAll(b);
            else duoQueue.add(b);
            return;
        }
    ...
    private void broadcastJoin(Set<java.util.UUID> team) {
        ...
        Bukkit.broadcastMessage("§d[경쟁전] §f" + sb.toString() + " §7매칭 대기중 (듀오)");
    }
